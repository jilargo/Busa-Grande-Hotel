<?php
/** Registration page. Vars: $googleEnabled */
$old = session()->oldInput();
$name  = $old['name'] ?? '';
$email = $old['email'] ?? '';
?>
<h2 class="text-center font-display text-2xl font-semibold text-brand-950">Create your account</h2>
<p class="mt-1 text-center text-sm text-slate-500">One account for bookings, stays and receipts.</p>
...
<form method="post" action="<?= url('register') ?>" class="mt-7 space-y-4">
    ...
    <div>
        ...
        <input type="text" id="name" name="name" value="<?= e($name) ?>" required autofocus maxlength="120"
    ...
    <div>
        ...
        <input type="email" id="email" name="email" value="<?= e($email) ?>" required maxlength="190"
               class="w-full rounded-lg border border-slate-300 px-3.5 py-2.5 text-sm text-slate-800 placeholder-slate-400 focus:border-brand-600 focus:outline-none focus:ring-2 focus:ring-brand-600/20">
    </div>
...
<p class="mt-6 text-center text-sm text-slate-500">
    ...
    <a href="<?= url('login') ?>" class="font-semibold text-brand-700 hover:text-brand-900">Sign in</a>
</p>